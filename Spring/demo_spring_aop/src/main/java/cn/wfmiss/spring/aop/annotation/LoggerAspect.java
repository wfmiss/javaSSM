 ...
 * @AfterReturning：返回通知，在目标对象方法之后执行
 * @AferThrowing:异常通知，在目标i方法的catch字句中执行
 *
 * 2、切入点表达式：设置在标识通知的注解value属性中
 * execution(public int cn.wfmiss.spring.aop.annotation.CalculatorImpl.add(int,int))
 * execution(* cn.wfmiss.spring.aop.annotation.CalculatorImpl.*(..)
 * 第一个*表示任意的访问修饰和返回值
 * 第二个*表示类中任意的方法
 * ..表示任意的参数列表
 * 类的地方也可以使用*，表示包下所有的类
 ...
 * @Pointcut("execution(* cn.wfmiss.spring.aop.annotation.CalculatorImpl.*(..))")
 * public void pointCut(){}
 * 使用方式：@Before("pointCut()")
 *
 *
 * 4、获取点连接点的信息
 * 在通知方法的参数位置，设置JoinPoint类型的参数，即可以获取连接点所对应方法的信息
 * //获取连接点所对应签名信息
...
public class LoggerAspect {
    ...
    //@Before("execution(public int cn.wfmiss.spring.aop.annotation.CalculatorImpl.add(int,int))")
